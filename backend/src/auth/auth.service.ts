...
export class AuthService {
  ...
  async login(userDto: LoginDto) {
    ...
    if (!user.isVerifiedEmail) throw new HttpException('Необходимо потвердить почту', HttpStatus.BAD_REQUEST)
  ...
  async registration(userDto: CreateUserDto, avatar: any) {
    ...
    await this.userService.createUser({ ...userDto, password: hashPassword }, avatar)
    return {
      statusCode: 200,
